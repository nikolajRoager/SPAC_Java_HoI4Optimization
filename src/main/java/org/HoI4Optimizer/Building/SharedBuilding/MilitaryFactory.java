...
public class MilitaryFactory extends Factory implements Cloneable{
    ...
    public MilitaryFactory(State location, boolean underConstruction,double efficiencyBase)
    {
        ...
        statisticsDepartment = new DataLogger();
        openDepartment();
        statisticsDepartment.addPlot("Production stats ("+name+") in "+location.getName(),Map.of("Factory Output", Color.GREEN,"Efficiency Cap",new Color(64,255,255,127),"Efficiency",Color.CYAN,"Resource Multiplier",Color.RED),"%",false);
        statisticsDepartment.addPlot("Production ("+name+") in "+location.getName(),Map.of("Daily MIC", new Color(0,255,128),"Total MIC",Color.GREEN,"Units produced",Color.RED),"MIC/units",false);
    }
