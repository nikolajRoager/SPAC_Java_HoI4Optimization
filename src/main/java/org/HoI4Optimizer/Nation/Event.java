...
public class Event {
    ...
    public Event(String name, PropertyEvent.Target target, boolean bvalue)
    {
        ...
        properties.add(new PropertyEvent(target,bvalue?1.0:0.0,false));
    }
    ...
    public Event(String name, PropertyEvent.Target target, double dvalue, boolean add)
    {
        ...
        properties =new ArrayList<PropertyEvent>();
        states =new ArrayList<StateEvent>();
        properties.add(new PropertyEvent(target,dvalue,false));
    }
    ...
    /// Create a single property event, usually created from the commandline
    public Event(String name, PropertyEvent.Target target, Ideology idvalue)
    {
        this.name=name;
        properties =new ArrayList<PropertyEvent>();
        states =new ArrayList<StateEvent>();
        properties.add(new PropertyEvent(target,idvalue.ordinal(),false));
    }
    ...
    /*Package private intentionally*/ void apply(NationalProperties properties, List<State> NationalStates, int day, PrintStream out)
    {
        if (out!=null)
        {
        ...
        for (var stateEvent : this.states)
            NationalStates.get(stateEvent.stateID()).apply(stateEvent,properties,out);
    }
