...
{
    public LinePlotFrame(String name,String yAxisName,List<LinePlotData> lines,DataLogger.logDataType type)
    {
        ...
        for (int i = 0; i < lines.size(); i++)
        {
            ...
            {
                lineData.add(new Day(cal.getTime()),line.data().get(day)*(type== DataLogger.logDataType.Percent || type== DataLogger.logDataType.UnboundedPercent || type== DataLogger.logDataType.PositiveUnboundedPercent? 100.0:1.0));
                cal.add(Calendar.DAY_OF_YEAR, 1);
            }

            renderer.setSeriesPaint(i, line.color());
            renderer.setSeriesStroke(i,new BasicStroke(2.0f));
            tsc.addSeries(lineData);
        }
